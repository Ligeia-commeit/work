...
class SAINT(nn.Module):
    def __init__(self, num_items, num_skills, hidden_dim=128, num_heads=4, num_layers=2, dropout=0.5):
        """
        Sequential Attention Integer Network with Time模型
        ...
        self.time_embedding = nn.Embedding(10000, hidden_dim)  # 时间间隔嵌入
    ...
    def forward(self, item_seq, interaction_seq, time_seq):
        """
        前向传播
        ...
        Args:
            ...
            time_seq: 时间间隔序列，形状为(batch_size, seq_length)
        ...
        time_embedded = self.time_embedding(time_seq)
        ...
        # 输入表示
        input_embedded = item_embedded + interaction_embedded + time_embedded + position_embedded

        # Transformer编码
        encoder_output = self.encoder(input_embedded)

        # 预测
        output = self.sigmoid(self.output_layer(encoder_output))
    ...
    def fit(self, data, q_matrix=None, epochs=10, batch_size=32, learning_rate=0.001):
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(sequences), batch_size):
                ...
                time_seq = torch.zeros((len(batch_sequences), max_seq_length - 1), dtype=torch.long)
                ...
                for j, seq in enumerate(batch_sequences):
                    ...
                    if seq_length > 1:
                        ...
                        # 构建时间间隔序列（简化处理，实际应用中需要真实的时间间隔）
                        if 'time_intervals' in seq:
                            time_seq[j, :seq_length - 1] = torch.tensor(seq['time_intervals'][:-1], dtype=torch.long)
                ...
                # 前向传播
                output = self.forward(item_seq, interaction_seq, time_seq)
